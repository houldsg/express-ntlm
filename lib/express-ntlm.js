...
module.exports = function(options) {
    ...
    function handle_type3(request, response, next, ntlm_message, callback) {
        ...
        proxy.authenticate(ntlm_message, function(error, result) {
            console.log('auentication callback called', error, result);
            if (error) return callback(error);

                console.log('was successfull once' + user);
                var proxy = cache.get_proxy(request.connection.id);
                
                proxy.search(proxy.base, ["sAMAccountName", "HOULDSG"], ['sAMAccountName'], function(error, result){
                    console.log('greg', result, error);
                    next();
                });
