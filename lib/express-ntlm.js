        ...
        async.eachSeries(!options.domaincontroller ? [ -1 ] : typeof options.domaincontroller === 'string' ? [ options.domaincontroller ] : options.domaincontroller, function(server, eachDomaincontrollerCallback) {
            ...
            if (server === -1) {
                ...
                var serverurl = url.parse(server),
                    ...
                    port = options.port || serverurl.port || (use_tls ? 636 : 398);
                options.debug(options.prefix, 'Initiating connection to Active Directory server ' + serverurl.host + ', port: ' + port + ', use_tls: ' + use_tls + ' (domain ' + domain + ') using base DN "' + decoded_path + '".');
                proxy = new NTLM_AD_Proxy(serverurl.host, domain, decoded_path, port, use_tls, options.tlsOptions || {});
            } else {
