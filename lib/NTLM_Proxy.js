...
NTLM_Proxy.prototype.search = function(base, criteria, attributes, search_callback) {
    var _this = this;
    var result;
   
    this.socket.on('data', function(data) {
        if (search_callback) {
            search_callback(null, _this.proto.parse_search_resp(data));
    ...
    this.socket.on('error', function(error) {
        if (search_callback) {
            search_callback(error);
            search_callback = null;
        }
    });
    //console.log(base, criteria, attributes)
    var msg = this.proto.make_search_req(base, criteria, attributes);
    this._transaction(msg);
};
